...
module.exports = (lat1, lng1) => {
  ...
  for (let i = 0; i < airports.airports.length; i++) {
    let newD = distanceToAP(
      lat1,
      lng1,
      airports.airports[i].latitude_deg,
      airports.airports[i].longitude_deg
    );

    if (d === null) var d = newD;
    dArr.push({ d: newD ,  ap:airports.airports[i]});
  ...
  const slicedArr = dArr.slice(0, 4);
